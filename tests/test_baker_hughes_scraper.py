...
from scrapers.baker_hughes.rigs import run as run_rigs, _sha256_bytes, _select_current_weekly_link
...
async def test_baker_hughes_requests_selects_current_weekly(clean_baker_dir: None) -> None:
    """Test selects only the current weekly, avoiding archives, using requests.Session."""
    
    with patch("scrapers.baker_hughes.rigs.requests.Session") as mock_session_cls:
    ...
    assert "f506ff08" in result["path"] or True  # The url is not directly returned now, check hash.
    assert result["bytes"] > 1000


@pytest.mark.asyncio
async def test_baker_hughes_content_hash_staleness_gate(clean_baker_dir: None) -> None:
    """Test staleness gate skips when sha-256 hash matches via requests."""
