...
def test_publish_dashboard_trigger_graph() -> None:
    ...
    triggers = content.get("on", {}).get("workflow_run", {}).get("workflows", [])
...
def test_every_scraper_workflow_commits_its_health() -> None:
    scraper_health_expectations: dict[str, str] = {
        ...
        "gasnom.yml": "data/health/gasnom_${SLUG}.json",
...
def test_month_end_normalize_handles_iso_dates() -> None:
    cfg = {"month_end_normalize": True}

    # ISO month-start e.g. from eia_lng_exports
    d1 = normalize_period("2026-05-01", cfg)
    assert d1 == date(2026, 5, 31), f"Expected 2026-05-31, got {d1}"

    # Leap year February
    d2 = normalize_period("2024-02-01", cfg)
    assert d2 == date(2024, 2, 29), f"Expected 2024-02-29, got {d2}"

    # Bare YYYY-MM
    d3 = normalize_period("2026-06", cfg)
    assert d3 == date(2026, 6, 30), f"Expected 2026-06-30, got {d3}"
